...
bool CPosition::Closing(
   ...
   if (bool(cFlag&ORDER_PENDING)){
      ...
      cCloseOrder=new CDeal(cTradeConst.symbol,ENUM_ORDER_TYPE(1-_type),_volume,0.0,0.0,0.0,0,0,0,NULL,cTradeConst,0,0,false);
      ...
      if (OrderClose(OrderTicket(),OrderLots(),TradePrice(cTradeConst.symbol,-1),SHORT_MAX,clrAliceBlue)   &&
...
void CPosition::Close(double volume #ifdef MY_MQL_LIB_TRADE_LOG ,string from #endif){
   if (!IsOpen()||Compare(volume,_volume,cTradeConst.lotDigits)!=e_compareLess){
...
void CPosition::PositionStopsControl(void){
   if (!SelectPosition()) return;
      ...
      cProfit=OrderProfit();
   #endif
   double price=TradePrice(cTradeConst.symbol,-_direct);
   if (cTral!=NULL){
      double tralSL=cTral.GetSL(price,sl,_price);
      if (Compare(tralSL,_sl,cTradeConst.digits)!=e_compareEqually){
         cSLControl=_sl=tralSL;
         cFlag|=TRADE_SL_CHANGE_IN_PROCESS;
      }
   }
   if(Compare(tp,cTPControl,cTradeConst.digits)==e_compareEqually)
   ...
   if(Compare(sl,cSLControl,cTradeConst.digits)==e_compareEqually){
      cFlag&=~TRADE_SL_CHANGE_IN_PROCESS;
   }
   else if (!cIsFirstControl){
   ...
   if ((!sl||Compare(sl,cSLControl,cTradeConst.digits)!=0)&&cSLControl&&ComparePrice(price,cSLControl,-_direct,cTradeConst.digits)>=0) {
      isSLTPClose=true; cCloseFlag|=CLOSE_BY_SL;
   }
   if ((!tp||Compare(tp,cTPControl,cTradeConst.digits)!=0)&&cTPControl&&ComparePrice(price,cTPControl,_direct,cTradeConst.digits)>=0) {
      isSLTPClose=true; cCloseFlag|=CLOSE_BY_TP;
   }
   if (isSLTPClose) {Closing(#ifdef MY_MQL_LIB_TRADE_LOG __FUNCSIG__ #endif); return;}
   double modSL=IS_VIRTUAL_SL?sl:CheckPriceLevel(sl,price,cTradeConst.freezeLevel,cTradeConst.digits),
          modTP=IS_VIRTUAL_TP?tp:CheckPriceLevel(tp,price,cTradeConst.freezeLevel,cTradeConst.digits);
   if (!IS_VIRTUAL_SL&&Compare(_sl,sl,cTradeConst.digits)!=0){
      if (!sl||modSL!=0.0) modSL=!_sl?_sl:CheckPriceLevel(_sl,price,cTradeConst.stopLevel,cTradeConst.digits);
      if (!modSL&&_sl>0.0) modSL=sl;}
   if (!IS_VIRTUAL_TP&&Compare(_tp,tp,cTradeConst.digits)!=0){
      if (!tp||modTP!=0.0) modTP=!_tp?_tp:CheckPriceLevel(_tp,price,cTradeConst.stopLevel,cTradeConst.digits);
...
void CPosition::ModifyPosition(double mSL,double mTP){
      ...
      cRequest.symbol=cTradeConst.symbol;
...
//      if (!res) Print(cTradeConst.stopLevel," | ",Ask," | ",Bid," | ",_ticket," | ",price," | ",mSL," | ",mTP," | ",_sl," | ",_tp," | ",OrderStopLoss()," | ",OrderTakeProfit()," | ",OrderCloseTime());
   #endif}
//----------------------------------------------------------------------
...
void CPosition::NewSL(int mSL){
   ...
   cSLPips=mSL;
   if (_price){
      _sl=!mSL?0.0:_price-_direct*mSL*cTradeConst.point;
      cSLControl=_sl;
      cFlag|=TRADE_SL_CHANGE_IN_PROCESS;
   }
}
...
void CPosition::NewTP(int mTP){
   ...
   cTPPips=mTP;
   if (_price){
      _tp=!mTP?0.0:_price+_direct*mTP*cTradeConst.point;
      cTPControl=_tp;
      cFlag|=TRADE_TP_CHANGE_IN_PROCESS;
   }
}
...
bool CPosition::SetBreakEven(int mBE){
   ...
   double sl=_price+_direct*mBE*cTradeConst.point;
   if (ComparePrice(sl,_direct>0?Bid(cTradeConst.symbol):Ask(cTradeConst.symbol),_direct,cTradeConst.digits)>=0) return false;
   _sl=sl;
   cSLControl=_sl;
   cFlag|=TRADE_SL_CHANGE_IN_PROCESS;
   return true;}
...
void CPosition::NewSL(double mSL,double mPrice=0.0,bool mIsCancelIfError=true){
   #ifdef __MQL5__
      if (IsOpen()) mPrice=!mPrice?TradePrice(cTradeConst.symbol,-_direct):mPrice; 
      else{
         COrder::NewSL(mSL);
         return;}
   #else
      mPrice=!IsOpen()?cOrderPrice:!mPrice?TradePrice(cTradeConst.symbol,-_direct):mPrice;
   #endif
   if ((!mIsCancelIfError #ifndef __MQL5__ &&_type>O_T_SELL #endif)||(!mSL&&_sl>0.0)||ComparePrice(mSL,mPrice,_direct,cTradeConst.digits)<0){
...
void CPosition::NewTP(double mTP,double mPrice=0.0,bool mIsCancelIfError=true){
      ...
      mPrice=!IsOpen()?cOrderPrice:!mPrice?TradePrice(cTradeConst.symbol,-_direct):mPrice;
   #endif
   if ((!mIsCancelIfError #ifndef __MQL5__ &&_type>O_T_SELL #endif)||(!mTP&&_tp>0.0)||ComparePrice(mTP,mPrice,_direct,cTradeConst.digits)>0){
...
bool CPosition::CheckClosePosition(void){
      ...
      if (PositionSelectByTicket(_ticket)){
         ...
         if (Compare(_volume,volume,cTradeConst.digits)!=e_compareEqually){
            if (_volume!=0.0)
...
//----------------------------------------------------------------------------
   bool CPosition::CheckDealFull(){
      ...
      _price=cDealPrice;
      _sl=cOrderSL?cOrderSL:!cSLPips?0.0:NormalizePrice(_price-_direct*cSLPips*cTradeConst.point);
      _tp=cOrderTP?cOrderTP:!cTPPips?0.0:NormalizePrice(_price+_direct*cTPPips*cTradeConst.point);
      cSLControl=_sl;
      cTPControl=_tp;
      #ifdef __MQL5__
...
//---------------------------------------------------------------------------
   bool CPosition::SelectNettingPosition(void){
      if (!PositionSelect(cTradeConst.symbol)) return false;
      CheckChangePosition();
      return true;}
//-------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------
   bool CPosition::ChangePosition(double volume #ifdef MY_MQL_LIB_TRADE_LOG, string fromWhere #endif){
      if (NormalizeDouble(volume,cTradeConst.lotDigits)==0.0)
         return true;
      if (NormalizeDouble(_volume+volume,cTradeConst.lotDigits)==0.0){
      ...
      CDeal* deal=new CDeal(cTradeConst.symbol,type,MathAbs(volume),0.0,0.0,0.0,0,0,0,NULL,cTradeConst,0,0,false);
      if (!cIsNetting)
...
//-------------------------------------------------------------------------------------------
   bool CPosition::ChangePosition(double volume #ifdef MY_MQL_LIB_TRADE_LOG, string fromWhere #endif){
      ...
      if (!OrderSelect(_ticket,SELECT_BY_TICKET)) return false;
      if (OrderCloseTime()) CLOSE_VALUE_INIT
      if (OrderClose(OrderTicket(),MathAbs(volume),TradePrice(cTradeConst.symbol,-1),SHORT_MAX,clrAliceBlue)){
         CheckPartialClose();
         return true;
      }
      else

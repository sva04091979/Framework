...
private:
   ...
   double            NormalizePrice(double mPrice)    {return NormalizeDouble(MathRound(mPrice/_tickSize)*_tickSize,cTradeConst.digits);}
  };
...
CClassicTral* CClassicTral::Init(CTradeConst* mTradeConst,int mDirection){
   ...
   cTral=NormalizePrice(cTralPips*cTradeConst.point);
   cTralStep=NormalizePrice(cTralStepPips*cTradeConst.point);
   return GetPointer(this);}
//----------------------------------------------------------------------------------------------------
double CClassicTral::GetSL(double mPrice,double mSL,double mPriceOpen){
   if (cBreakEvenTrigger==EMPTY_VALUE) cBreakEvenTrigger=mPriceOpen+cDirection*cBreakEvenTriggerPips*cTradeConst.point;
   if (cBreakEven==EMPTY_VALUE) cBreakEven=mPriceOpen+cDirection*cBreakEvenPips*cTradeConst.point;
   return cBreakEvenTriggerPips!=INT_MIN&&cBreakEvenPips!=INT_MIN&&(!mSL||ComparePrice(mSL,cBreakEven,cDirection,cTradeConst.digits)<0)?BreackEvenControl(mPrice,mSL):TralControl(mPrice,mSL);}
//----------------------------------------------------------------------------------------------------
double CClassicTral::BreackEvenControl(double mPrice,double mSL){
   return ComparePrice(mPrice,cBreakEvenTrigger,cDirection,cTradeConst.digits)<0?mSL:cBreakEven;}
//---------------------------------------------------------------------------------------------------
double CClassicTral::TralControl(double mPrice,double mSL){
   if (!cTralPips) return mSL;
   double res=!mSL||CompareDouble(cDirection*(mPrice-mSL),cTral,cTradeConst.digits)>=0?NormalizePrice(mPrice-cDirection*cTral):mSL;
   return !mSL||CompareDouble(cDirection*(res-mSL),cTralStep,cTradeConst.digits)>=0?res:mSL;
}